...
mkdir workspace/${APP_BACK}
sed -i 's/.*#APP_BACK/    - .\/workspace\/'"${APP_BACK}"':\/node\/src/g' docker-compose.yml

docker-compose run embercli ember new ${APP_WEB}
docker-compose run nodejscli yo angular-fullstack ${APP_BACK}

sed -i 's/.*#APP_WEB/    - .\/workspace\/'"${APP_WEB}"':\/node\/src/g' docker-compose.yml
